...
def generate_report(a1, a2, output_dir):
    ...
    n1, n2 = a1.stats['num_nodes'], a2.stats['num_nodes']
    e1, e2 = a1.stats['num_edges'], a2.stats['num_edges']
    r.append(f"\nSCALE: {a2.name if n2>n1 else a1.name} has {max(n1,n2)/min(n1,n2):.1f}x more nodes")
    r.append(f"COMPLEXITY: {a2.name if a2.stats['branch_ratio']>a1.stats['branch_ratio'] else a1.name} is more complex")
    r.append(f"N50: {a1.name}={a1.stats['n50']:,} vs {a2.name}={a2.stats['n50']:,}")
    r.append("=" * 80)
    txt = "\n".join(r)
    print(txt)
    with open(f'{output_dir}/comparison_report.txt', 'w') as f: f.write(txt)
    json_data = {'graph1': {'name': a1.name, 'stats': {k: float(v) if isinstance(v, (int, float, np.integer, np.floating)) else v for k, v in a1.stats.items()}}, 'graph2': {'name': a2.name, 'stats': {k: float(v) if isinstance(v, (int, float, np.integer, np.floating)) else v for k, v in a2.stats.items()}}, 'generated': datetime.now().isoformat()}
    with open(f'{output_dir}/comparison_data.json', 'w') as f: json.dump(json_data, f, indent=2)
    print(f"\nReport: {output_dir}/comparison_report.txt")
    print(f"JSON: {output_dir}/comparison_data.json")
...
def main():
    ...
    name1 = "Graph Iteration 1"
    name2 = "Fully Converged Graph (HPRC)"
    print("\n" + "="*60)
